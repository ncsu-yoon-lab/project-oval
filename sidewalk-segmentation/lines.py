...
def process_image(img):
    ...
    # # Taking only the left quarter of the original image
    new_width = int(width/4)
    left_quarter_img = np.zeros((height, new_width, 3), dtype=np.uint8)
    left_quarter_img[:,0:new_width,:] = img[:,0:new_width,:]

    # # Taking only the right quarter of the original image
    right_quarter_img = np.zeros((height, new_width, 3), dtype=np.uint8)
    right_quarter_img[:,0:new_width,:] = img[:,new_width*3:width,:]

    # Removing the center part of the original image
    processed_img = cv2.hconcat((left_quarter_img, right_quarter_img))
    # cv2.imshow('removed middle', processed_img)

    # # "Horse blinder" mask
    mask = np.zeros_like(processed_img)
    vertices = [np.array([[0, 480], [0, 350], [340, 150], [640, 350], [640, 480]], dtype=np.int32)]
    cv2.fillPoly(mask, vertices, (255, 255, 255))
    processed_img = cv2.bitwise_and(processed_img, mask)

    # Filter based on color (focus on yellow)
    img_as_hsv = cv2.cvtColor(processed_img, cv2.COLOR_BGR2HSV)
    yellow_mask = cv2.inRange(processed_img, lower_yellow_threshold, upper_yellow_threshold)
    yellow_masked = cv2.bitwise_and(processed_img, processed_img, mask=yellow_mask)
    processed_img = yellow_masked
