...
async def stream_microphone_and_handle(session: any, p: pyaudio.PyAudio, gui_queue: queue.Queue) -> None:
            ...
            try:
                async for message in session.receive():
                    ...
                    if hasattr(message, "tool_call") and message.tool_call:
                        ...
                        asyncio.create_task(handle_tool_calls(session, message.tool_call, gui_queue, turn_ended))
...
async def handle_tool_calls(session: any, tool_call: any, gui_queue: queue.Queue, turn_ended: asyncio.Event) -> None:
...
async def run_async_logic(gui_queue: queue.Queue) -> None:
        ...
        while True:
            ...
            try:
                async with client.aio.live.connect(model=MODEL, config=base_config) as session:
                    ...
                    await stream_microphone_and_handle(session, p, gui_queue)
            except Exception as e:
...
def main():
    ...
    def run_loop():
        ...
        try:
            loop.run_until_complete(run_async_logic(gui_queue))
        finally:
            loop.close()
