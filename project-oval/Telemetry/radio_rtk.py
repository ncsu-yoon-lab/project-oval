...
class GPSRadioTXNode(Node):
    ...
    def rtk_callback(self, msg):
        if not np.isnan(msg.latitude) and not np.isnan(msg.longitude):
            ...
            self.get_logger().info(f"Received RTK GPS: {self.latest_lat}, {self.latest_lon}")
        else:
    ...
    def send_data(self):
        if self.latest_lat is not None and self.latest_lon is not None:
            lat_int = int(self.latest_lat * 1e6)
            lon_int = int(self.latest_lon * 1e6)
            data = struct.pack('!ii', lat_int, lon_int)
            ser.write(data)
            self.get_logger().info(f"Sent binary: {data.hex()} (lat={self.latest_lat}, lon={self.latest_lon})")
        else:
