...
def logging_switch_callback(data):
	global log
	log = (log + 1) % 2	#for now we have only two options for logging: start and stop
...
def main(args=None):
	...
	subscription_logging_switch = node.create_subscription(Int64 , "logging_switch" , logging_switch_callback , 1)		
	...
	while rclpy.ok():
		...
		if mode == 0:
			# manual mode
			pwm_throttle = pwm1(throttle)
			pwm_steer = pwm2(steer)

			# if logging is meant to start, but we are in manual, give manual values
			if log == 1:
				print("manual throttle: %d, manual steering: %d" % (pwm_throttle, pwm_steer))

		else:
			# auto mode
			pwm_throttle = pwm1(auto_throttle)
			pwm_steer = pwm1(pid_steer)

			# if logging is meant to start, but we are in manual, give manual values
			if log == 1:
				print("auto throttle: %d, auto steering: %d" % (auto_throttle, pid_steer))

		# # start logging the throttle and steering
		# if log == 1:
		# 	print("auto throttle: %d, auto steering: %d" % (auto_throttle, pid_steer))
		...
		# stdscr.addstr(4, 5, 'Auto Throttle: %.2f  ' % auto_throttle)
		# stdscr.addstr(5 , 5 , 'PID Steering: %.2f' % pid_steer)
		# f.write(str(time.time()) + "+" + str(pwm_steer) + '/n')
		...
		if safe_distance_violation:
			print("-- Safe distance violation (%.2f m)--" % (lidar_min_dist))
		else:
			print("                             ")
		if stop_sign_message:
			print(stop_sign_message)
		else:
			print("                             ")
		print("/n")
