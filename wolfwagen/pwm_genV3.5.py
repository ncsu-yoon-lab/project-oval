...
def main(args=None):
    ...
    pub_ld_turnval = node.create_publisher(Int64, 'ld_turn_val', 1)
    sub_astar_turnval = node.create_subscription(Int64, 'astar_turn_val', astar_turn_callback, 1)
    ...
    while rclpy.ok():
        ...
        if mode == 0:
            ...
            pwm_throttle = pwm(auto_throttle)
            pwm_steer = int(pid_steer)
            pub_ld_turnval(astar_turn_val)
            pub_astar_turnsig(ld_turn_sig)
